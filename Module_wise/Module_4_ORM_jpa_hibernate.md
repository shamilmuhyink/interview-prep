...
### Q16. 🌊 Propagation in Hibernate and how to achieve it

**Transaction propagation defines how transactions relate to each other when one transactional method calls another. In Spring (often used with Hibernate), this is managed by the `@Transactional` annotation.**

Types of propagation:
- **REQUIRED (Default)**: If a transaction exists, join it. Otherwise, create a new one.
- **REQUIRES_NEW**: Suspend the current transaction (if any) and create a new transaction.
- **MANDATORY**: Requires an existing transaction; throws an exception if none exists.
- **SUPPORTS**: Executes within the current transaction if one exists; otherwise, executes non-transactionally.
...
- **NESTED**: Executes within a nested transaction (savepoint) if a current transaction exists.

**How to achieve it**: Use Spring's `@Transactional(propagation = Propagation.REQUIRES_NEW)` on the service layer methods.

---

### Q17. 🔍 Difference between get() and load()

**In Hibernate, both are used to fetch a single entity by its primary key, but they differ in their execution:**

- **`get()`**: 
  - Hits the database immediately (if the object is not in the first-level cache).
  - Returns `null` if the object is not found.
  - Used when you are not sure if the object exists in the database.
- **`load()`**: 
  - Returns a proxy object (a placeholder) and only hits the database when a property of the object (other than the ID) is accessed (lazy loading).
  - Throws `ObjectNotFoundException` if the object doesn't exist when the proxy is initialized.
  - Used when you are certain the object exists in the database and you might just want its reference (e.g., to set a foreign key relation without fetching the whole object).

---

### Q18. 🏭 Use of session and sessionFactory in Hibernate

- **SessionFactory**: 
  - It is a factory class for `Session` objects.
  - It is a heavy-weight, thread-safe, and immutable object created during application startup.
  - There is typically only one `SessionFactory` per database in an application.
  - It caches SQL statements and other mapping metadata.
- **Session**:
  - It represents a single unit of work (a single database connection) and is used to perform CRUD operations.
  - It is light-weight, not thread-safe, and short-lived (usually created and destroyed per request or transaction).
  - It holds the first-level cache (L1 cache) of persistent objects.

---

### Q19. 🚀 Caching and types of it in Hibernate

**Caching in Hibernate improves application performance by reducing the number of database queries.**

Types of caching:
- **First-Level Cache (L1 Cache)**:
  - Enabled by default.
  - Associated with the `Session` object.
  - Scope is limited to the current session; once the session is closed, the cache is lost.
- **Second-Level Cache (L2 Cache)**:
  - Disabled by default (must be explicitly enabled).
  - Associated with the `SessionFactory` object.
  - Scope is application-wide; it is shared across all sessions.
  - Providers include Ehcache, Hazelcast, Infinispan.
- **Query Cache**:
  - Disabled by default.
  - Caches the results of a specific query (HQL/Criteria) along with their parameters.
  - Usually used in conjunction with the Second-Level Cache for optimal performance.
